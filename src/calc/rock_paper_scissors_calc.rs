...
pub fn calculate_rock_paper_scissors(games: &Vec<Game>) -> RockPaperScissorStats {
    ...
    for game in games {
        let mut winner: Entity = Entity::Player(game.first_throw().thrower);
        let mut loser: Entity = Entity::Player(game.first_throw().runner);
        if game.won_rps_but_decided_to_run{
            let tmp = winner;
            winner = loser;
            loser = tmp;
        }
