...
pub fn calc_team_first_throws(games: &Vec<Game>) -> FirstTeamThrowsStatistics {
    ...
    for game in games {
        let team_id_going_first = team_from_player(game.first_throw().thrower.id(), game)
            .named_entity
            .to_owned();
    ...
    for game in games {
        ...
        let team_going_second = team_from_player(game.first_throw().runner.id(), game)
            .named_entity
            .to_owned();
        let throws = first_throws
...
mod tests {
    ...
    use crate::util::test::{convert_first_throw_games, game_2nd_finish, game_2nd_finish_after_enemy_began, game_2nd_finish_right_began};
    #[test]
